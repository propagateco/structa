...
`sst dev` serves the app directly from Vite on `http://localhost:3000` —
no reverse proxy required. (An earlier Caddy HTTP/2 proxy existed for
Electric SQL's long-polling shapes; it was removed with the Electric
decommission.)

**Usage:** access the app at `http://localhost:3000/app`. All agent-browser
commands and tests should use port 3000.
