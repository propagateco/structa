 ...
 * Get these from your Loops dashboard: Settings > API > Mailing Lists
 */
export const MAILING_LISTS = {
	/** General marketing emails */
	MARKETING: "cmlr5hoqd01br0iwi1mctc2ju",
	/** Product updates */
	PRODUCT: "cmlr5ipn400ux0iwd72wif6y2",
	/** Waitlist/early access */
	WAITLIST: "cmlrr80fk1tm60is12m3g5aro",
} as const;
...
function handleLoopsError(
	...
	if (error instanceof APIError) {
		...
		const errorMessage =
			...
			errorDetails.error ||
			`Loops API returned ${error.statusCode}`;

		console.error(`${LOG_PREFIX} ${timestamp()} FAILED: ${errorMessage}`);
		console.error(
			`${LOG_PREFIX} ${timestamp()} Error details:`,
...
export async function createContactInLoops({
	...
	try {
		...
		// Unexpected response
		console.error(
			`${LOG_PREFIX} ${timestamp()} UNEXPECTED RESPONSE:`,
			response,
		);
		return {
			success: false,
...
export async function sendEventInLoops({
	...
	try {
		...
		if (response.success) {
			console.log(
				`${LOG_PREFIX} ${timestamp()} SUCCESS: Event ${eventName} sent`,
			);
 ...
 */
export async function testLoopsApiKey(): Promise<
	LoopsResult<{ teamName: string }>
> {
	...
	try {
		...
		if (response.success && response.teamName) {
			console.log(
				`${LOG_PREFIX} ${timestamp()} SUCCESS: API key valid for team "${response.teamName}"`,
			);
...
export async function findContactInLoops({
	...
	console.log(
		`${LOG_PREFIX} ${timestamp()} Finding contact: email=${email}, userId=${userId}`,
	);
	...
	try {
		...
		if (response && response.length > 0) {
			console.log(
				`${LOG_PREFIX} ${timestamp()} SUCCESS: Found contact with id=${response[0].id}`,
			);
