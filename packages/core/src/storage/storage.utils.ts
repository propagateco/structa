...
export function buildObjectKey(object: ObjectReference): string {
	const relative = object.relativeDirectory
		? `/${object.relativeDirectory}`
		: "";
...
export function getFileMimeContentType(dataURL: string): string {
	...
	if (!mime) {
		throw new Error(
			"Invalid data URL: No MIME type found, defaulting to image/png.",
		);
	}
	return mime;
}
...
export function assertFileSize(sizeInBytes: number, maxBytes: number): void {
	if (sizeInBytes > maxBytes) {
		throw new Error(
			`File size exceeds the maximum limit of ${maxBytes} bytes.`,
		);
	}
}
