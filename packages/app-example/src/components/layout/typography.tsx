...
const HeaderMain = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
	({ className, ...props }, ref) => (
...
>(({ className, ...props }, ref) => (
	<h2 ref={ref} className={cn('text-base text-muted-foreground', className)} {...props} />
));
HeaderSubSection.displayName = 'HeaderSubSection';

const HeaderButtons = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
	({ className, ...props }, ref) => (
		<div ref={ref} className={cn('flex justify-end gap-2 transition-all duration-150', className)} {...props} />
	)
);
