...
export const brandingSavingSnapshotAtom = atom<BrandingModel.BrandingContextType | null>(null);
...
export const clearSavedChangesAtom = atom(
  ...
  (get, set, savedData: BrandingModel.BrandingContextType) => {
    ...
    for (const [key, currentValue] of Object.entries(currentChanges)) {
      const savedValue = savedData[key as keyof BrandingModel.BrandingContextType];
