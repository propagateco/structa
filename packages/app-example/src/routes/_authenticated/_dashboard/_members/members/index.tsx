...
import { ExternalLink, Search, Users, DollarSign, UserCheck, Clock } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { MemberDetailModal } from "@/components/members/member-detail-modal";

export const Route = createFileRoute("/_authenticated/_dashboard/_members/members/")({
...
const columns: ColumnDef<Member>[] = [
    ...
    cell: ({ row }) => {
      ...
      const getMainStatusInfo = (status: string) => {
        switch (status) {
          ...
          default:
            return { text: status.replace(/_/g, " "), variant: "secondary" as const };
        }
      };
      ...
      const getTooltipContent = () => {
        ...
        if (isDelinquent) {
          if (content) content += "\n\n";
          content += "Payment Issues: Recent payment attempts have failed. Update payment method or contact customer.";
        }
  ...
  },
  {
    ...
    header: "Product",
    cell: ({ row }) => {
      const productName = row.getValue<string | null>("subscriptionProductName");
      return productName || "—";
    },
  },
  {
...
function RouteComponent() {
  ...
  const updateQueryParams = (updates: Partial<MembersQueryParams>) => {
    setQueryParams(prev => ({
      ...
      offset: updates.search !== undefined || updates.status !== undefined ? 0 : prev.offset,
    }));
  };
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {isLoadingStats ? "—" : `$${stats?.monthlyRevenue?.toLocaleString() || "0"}`}
            </div>
          </CardContent>
        </Card>
          ...
          <div className="flex items-center justify-center h-full py-16">
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Members will appear here once customers subscribe to your products
              </p>
            </div>
          </div>
        }
