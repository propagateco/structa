...
import { Crosshair } from "../layout/Crosshair";
...
const LogoShowcase = ({
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			...
			// Stage 1: Start fading out current logo
		...
		<div className={cn("w-full", className)}>
			<div className={cn("grid w-full", "grid-cols-4 md:grid-cols-8")}>
				{currentLogos.map((logo, index) => (
					...
					>
						{index === 0 && (
							<Crosshair position="top-left" className="lg:hidden" />
						)}
						{index === 5 && (
							<Crosshair position="bottom-right" className="lg:hidden" />
						)}
						{index === 1 && (
							<Crosshair position="top-left" className="hidden lg:block" />
						)}
						{index === 4 && (
							<Crosshair position="bottom-right" className="hidden lg:block" />
						)}
