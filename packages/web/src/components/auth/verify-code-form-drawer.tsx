...
interface VerifyCodeFormDrawerProps {
	email: string;
	onBack: () => void;
	onSuccess: () => void;
}
...
/**
 * VerifyCodeFormDrawer - Drawer-optimized verification form
 *
 * Displays email being verified and handles code input
 * Uses callback pattern instead of navigation
 */
			...
			<Form {...form}>
				<form onSubmit={(e) => e.preventDefault()} className="space-y-2 w-full">
					...
					<Button
						type="button"
						variant="link"
						className="text-muted-foreground mt-10 px-0"
						onClick={onBack}
						disabled={isLoading}
						isLoading={isLoading}
